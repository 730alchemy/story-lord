...
def _format_prior_context(
    ...
    Includes:
    - All beats from all previous plot events
    - Beats 0..current_beat_idx-1 from the current event
    """
...
def _format_prior_narration(narrations: list[BeatNarration]) -> str:
...
def generate_narration(input_data: NarratorInput) -> NarratedStory:
    """
    ...
    for event_idx, plot_event in enumerate(plot_events):
        ...
        for beat_idx, beat in enumerate(plot_event.beats):
            ...
            generation_context = {
                ...
                "involved_characters": _format_involved_characters(
                    beat, input_data.characters
                ),
                "tone": input_data.tone,
                "prior_context": _format_prior_context(
                    prior_events, plot_event, beat_idx
                ),
                "prior_narration": _format_prior_narration(all_narrations),
            }
            ...
            for iteration in range(2, 4):
                eval_context = {
                    ...
                    "beat_type": beat_type,
                    "beat_description": beat_description,
                    "full_corpus": _format_prior_narration(all_narrations),
                    "prior_context": _format_prior_context(
                        prior_events, plot_event, beat_idx
                    ),
                }
